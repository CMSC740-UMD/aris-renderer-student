...
class BrdfQuery:
    ...
    For sampling, caller sets wo in local coordinates, and BRDF sets sampled wi, values, pdf, and is_specular.
    Also, sampled values are computed as (eval values) / pdf * cos(theta).

    For eval, caller sets wo and wi. BRDF sets values, pdf, and is_specular.
    """
    wo: Tensor  # (N, 3)
